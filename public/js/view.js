var View = (function () {
  ...
  function init() {
    ...
    //     map: THREE.ImageUtils.loadTexture('assets/wallpaper.jpg')
    //   })
    // );
    // sphere.scale.x = -1;
    // scene.add(sphere);
  ...
  function displayData(user, data) {
        ...
        var generateSphere = function (userData, texture) {
          ...
          if(userData.from_freq > 0) {
            createLine(user.position, main.position, userData.from_freq);
          }
          if(userData.to_freq > 0) {
            createLine(main.position, user.position, userData.to_freq);
          }
        };
